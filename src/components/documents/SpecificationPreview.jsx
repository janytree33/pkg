...
const SpecificationPreview = forwardRef(({ product, versionIndex, certNo, remark, issueDate, showEvalResult, showFinalGrade, docLanguage }, ref) => {
   ...
   * @param {boolean} forceHideEval - 강제로 평가 결과를 숨길지 여부 (비대상 부자재용)
   */
  const renderRows = (items, prefixKey, forceHideEval = false) => {
    ...
    const createRow = ({ bItem, comp }, idx, groupName) => {
      ...
      return (
        <tr key={`${prefixKey}-${groupName}-${idx}`} style={{ borderBottom: '1px solid #e5e7eb' }}>
          ...
          {(showEvalResult && !forceHideEval) && <td style={{ padding: '5px 4px', color: '#3b82f6', fontWeight: '600', wordBreak: 'break-all', fontSize: '10px' }}>
            {isKo 
              ? (comp.materialEvalResult === '대상제외' || forceHideEval === false && groupName === 'exempt' ? '대상제외' : comp.materialEvalResult || '미평가')
              : (comp.materialEvalResult === '재활용 최우수' ? 'Best' :
                 comp.materialEvalResult === '재활용 우수' ? 'Excellent' :
                 comp.materialEvalResult === '재활용 보통' ? 'Normal' :
                 comp.materialEvalResult === '재활용 어려움' ? 'Difficult' : 
                 comp.materialEvalResult === '대상제외' || groupName === 'exempt' ? 'Exempt' : 'Pending')}
          </td>}
    ...
    if (filling.length > 0) {
      rows.push(
        <tr key={`${prefixKey}-hdr-fill`} style={{ backgroundColor: '#f3f4f6', borderBottom: '1px solid #e5e7eb' }}>
          <td colSpan={(showEvalResult && !forceHideEval) ? "9" : "8"} style={{ padding: '4px 12px', fontWeight: 'bold', color: '#374151', fontSize: '10px', letterSpacing: '0.5px' }}>
    ...
    if (packaging.length > 0) {
      rows.push(
        <tr key={`${prefixKey}-hdr-pack`} style={{ backgroundColor: '#f3f4f6', borderBottom: '1px solid #e5e7eb' }}>
          <td colSpan={(showEvalResult && !forceHideEval) ? "9" : "8"} style={{ padding: '4px 12px', fontWeight: 'bold', color: '#374151', fontSize: '10px', letterSpacing: '0.5px' }}>
    ...
    if (rows.length === 0) {
      rows.push(
        <tr key={`${prefixKey}-empty`}>
          <td colSpan={(showEvalResult && !forceHideEval) ? "9" : "8"} style={{ padding: '10px', textAlign: 'center', color: '#9ca3af', fontSize: '10px' }}>
      ...
      {exemptItems.length > 0 && (
        <div style={{ marginBottom: '10px' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '10px', tableLayout: 'fixed' }}>
            <thead>
              {showEvalResult ? (
                <tr style={{ backgroundColor: '#f9fafb', borderTop: '1px solid #e5e7eb', borderBottom: '1px solid #e5e7eb' }}>
                  ...
                  <th style={{ padding: '6px 4px', fontWeight: '600', color: '#9ca3af', textAlign: 'left', width: '12%' }}>{isKo ? '재질·구조 평가' : 'Recyclability Assessment'}</th>
            ...
            <tbody>
              {renderRows(exemptItems, 'exempt', false)}
            </tbody>
          </table>
        </div>
      )}
